...
interface VendorPerformanceUpdate {
  delivery_score?: number
  quality_score?: number
  price_score?: number
  notes?: string
...
export const vendorsApi = {
  ...
  getByTrade: async (trade: string): Promise<Vendor[]> => {
    const response = await apiClient.get(`/vendors/by-trade/${trade}`)
    return response.data
  },

  getExpiringInsurances: async (days: number = 30): Promise<Vendor[]> => {
    const response = await apiClient.get(`/vendors/expiring-insurance/${days}`)
    return response.data
  },

  // Vendor Performance methods
  recordPerformance: async (vendorId: string, data: VendorPerformanceCreate): Promise<VendorPerformance> => {
    const response = await apiClient.post(`/vendors/${vendorId}/performance`, data)
    return response.data
  },
  ...
  updatePerformance: async (
    ...
    data: VendorPerformanceUpdate
  ): Promise<VendorPerformance> => {
    const response = await apiClient.put(`/vendors/${vendorId}/performance/${performanceId}`, data)
    return response.data
  },

  getVendorAnalytics: async (vendorId: string): Promise<{
    averageDeliveryScore: number
    averageQualityScore: number
    averagePriceScore: number
    totalProjects: number
    performanceHistory: VendorPerformance[]
  }> => {
    const response = await apiClient.get(`/vendors/${vendorId}/analytics`)
    return response.data
  },

  getAllVendorsAnalytics: async (): Promise<{
    topVendorsByRating: Vendor[]
    topVendorsByTrade: Record<string, Vendor[]>
    averageRatingByTrade: Record<string, number>
  }> => {
