...
export default function ContactsPage() {
  ...
  const projectContacts = mockContacts.filter(c => c.projectId === projectId)
  const filteredContacts = projectContacts.filter(c => {
    ...
    <Box>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField fullWidth select label="Contact Type" margin="normal" required>
            {contactTypes.map(type => (
              <MenuItem key={type.value} value={type.value}>{type.label}</MenuItem>
            ))}
          </TextField>
          ...
          <TextField fullWidth label="Role Description" margin="normal" multiline rows={2} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDialogOpen(false)}>Cancel</Button>
          <Button variant="contained" onClick={() => setDialogOpen(false)}>Add Contact</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
