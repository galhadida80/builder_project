...
export default function ExecutiveDashboardDark() {
  ...
  if (loading) {
    return (
      <Box sx={{ p: 3, bgcolor: '#0f172a', minHeight: '100vh' }}>
        <Skeleton variant="text" width={300} height={48} sx={{ mb: 1, bgcolor: '#334155' }} />
        <Skeleton variant="text" width={400} height={24} sx={{ mb: 4, bgcolor: '#334155' }} />
        <Grid container spacing={3} sx={{ mb: 3 }}>
          {[...Array(4)].map((_, i) => (
            <Grid item xs={12} sm={6} md={3} key={i}>
              <Skeleton variant="rounded" height={140} sx={{ borderRadius: 3, bgcolor: '#334155' }} />
            </Grid>
          ))}
        </Grid>
        <Grid container spacing={3}>
          {[...Array(2)].map((_, i) => (
            <Grid item xs={12} md={6} key={i}>
              <Skeleton variant="rounded" height={400} sx={{ borderRadius: 3, bgcolor: '#334155' }} />
            </Grid>
          ))}
        </Grid>
      </Box>
    ...
    <Box sx={{ p: 3, bgcolor: '#0f172a', minHeight: '100vh' }}>
            ...
            <Box sx={{ textAlign: 'center', width: '100%' }}>
              ...
              >
                {[
                  ...
                  { label: 'Quality Score', value: 94, color: '#0284c7' },
                  { label: 'Efficiency', value: 87, color: '#a78bfa' },
                ].map((metric) => (
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <ChartContainer>
            <Box sx={{ textAlign: 'center', width: '100%' }}>
              <Typography variant="h6" sx={{ fontWeight: 600, mb: 3, color: '#f1f5f9' }}>
                Monthly Trend
              </Typography>
              <Box
                sx={{
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-around', mt: 1 }}>
                {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug'].map((month) => (
                  <Typography key={month} variant="caption" sx={{ color: '#94a3b8' }}>
                    {month}
                  </Typography>
                ))}
              </Box>
